...
class sqlee{
	...
	function __construct($db,$config_path=null){
		$this->db = $db;
		if($_POST['restart'] || $_POST['delete'] == 'Cancel') die(self::reload_form());		
		...
		// add entire path to the config file if not in same file folder
		$system_folder = ($config_path != NULL? $config_path:'');
		$class_conf_path = $config_path. get_class($this) . '_conf';
	...
	public function reload_form($refresh)
	{
	// this class also exists in a few related libraries like conf_config and of course myparseframework
		unset($_POST);die(header("Refresh: $refresh;"));	
	}
	
	public function band_aid($query,$mode=NULL){
	// from myparse, simple mysqli query interface
		if ($mode == 2) {
	...
	public function record_editor($arr,$arr2,$ex=NULL,$form_filter=NULL){
	// i had to change this function call because myparse sends an array, this makes it slightly simpler for the brave developer
